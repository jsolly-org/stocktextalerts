...
- Always regenerate `seed.sql` after updating `scripts/users.json` or `scripts/us-assets.json`
...
The asset data is imported from `scripts/us-assets.json`. To update the asset list:

### JSON Structure

The `scripts/us-assets.json` file must follow this structure:
...
See `scripts/us-assets.json` for the canonical schema and example data.

### Update Process

1. Run `npm run db:fetch-assets` to fetch updated asset data from Finnhub, or update `scripts/us-assets.json` manually
