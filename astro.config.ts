 ...
 */
const useNodeAdapter = env.ASTRO_ADAPTER === "node" || process.env.ASTRO_ADAPTER === "node";
...
const maxDurationRaw = env.VERCEL_MAX_DURATION || process.env.VERCEL_MAX_DURATION;
const maxDurationParsed = maxDurationRaw ? Number.parseInt(maxDurationRaw, 10) : Number.NaN;
const vercelMaxDurationSeconds = maxDurationParsed > 0 ? maxDurationParsed : undefined;
...
export default defineConfig({
	...
	adapter: useNodeAdapter
		...
		: vercel({
				...(vercelMaxDurationSeconds ? { maxDuration: vercelMaxDurationSeconds } : {}),
			}),
