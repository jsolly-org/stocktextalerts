...
3. **Create followup PRs** — For each task, a parallel job checks out `dev`, runs the agent to implement the task (leaving changes uncommitted), runs pre-commit checks, and — if successful — commits, pushes to a branch, and creates a PR against `dev`.
