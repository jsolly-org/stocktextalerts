...
- **Happy path coverage only**: Focus on success paths and essential validation; avoid exhaustive edge-case coverage that increases maintenance cost. Exceptions: security tests (`.security.test.ts` suffix) may keep negative-path cases when they validate rejection of invalid input (e.g. `tests/api/auth/email/forgot-password.security.test.ts` describe "A user requests a password reset email from the forgot password form." / it "The request is rejected when the form is incomplete.").
...
- **Shared stubs**: Consolidate reusable notification stubs in `tests/helpers/shared-utils.ts` and reset between tests so mocks stay minimal and consistent.
...
## Test Structure
- **Use shared utilities**: Reuse helpers from `tests/helpers/shared-utils.ts` (and domain-specific utils like `tests/helpers/stock-update.ts`) to keep tests DRY and setup consistent.
