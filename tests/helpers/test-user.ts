...
export function createTestEmail(prefix = "test"): string {
	...
	// When email provider is live, use the SES mailbox simulator address.
	// Plus-addressing gives each test a unique address that SES accepts.
	...
	if (isLiveProviderEnabled("email")) {
		...
		const maxPrefixLength = 64 - "success+".length - 1 - shortId.length;
		const cappedPrefix = normalizedPrefix.slice(
			0,
			Math.max(1, maxPrefixLength),
		);
		return `success+${cappedPrefix}-${shortId}@simulator.amazonses.com`;
	}
