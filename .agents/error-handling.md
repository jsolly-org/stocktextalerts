...
- **Env var validation**: Don't add presence checks — `src/middleware.ts` validates all required env vars on every request. Only validate format/type or handle optional env vars.
