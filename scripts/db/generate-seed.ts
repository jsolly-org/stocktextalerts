...
async function generateUsersSql(
  ...
  // If `scripts/users.json` is present, the generated `supabase/seed.sql` will create auth users
...
async function main() {
  ...
  if (users.length > 0 && !isLocalSupabase) {
    rootLogger.warn(
      "Seeding auth users from scripts/users.json into a non-local Supabase project. These users will be created with a password derived from DEFAULT_PASSWORD.",
  ...
  If scripts/users.json exists, this seed includes auth user creation with passwords derived from DEFAULT_PASSWORD.
