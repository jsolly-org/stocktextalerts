...
async function generateUsersSql(
  ...
  // Security note: the generated `supabase/seed.sql` includes auth user creation and password hashing derived
  // from `DEFAULT_PASSWORD` (from `.env.local`). Treat both `.env.local` and `supabase/seed.sql` as sensitive
  // and do not use real production passwords here.
...
async function main() {
  ...
  Security: includes auth user creation and password hashing derived from DEFAULT_PASSWORD.
*/
