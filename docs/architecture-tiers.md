...
- Inbound webhooks (Telegram bot) — stable URLs on the app domain; CSRF middleware exempts these paths
...
| --- | --- | --- |
| `schedule` | Every 1 minute | Precise notification delivery and price-alert processing; Massive Starter quotes may be delayed up to 15 minutes. Human email/Telegram stay on 04:30–19:30 ET; stock-buyer `lambda` flat alerts use the Massive-capped equity window 04:00–20:00 ET. Session-day 09:00 ET also async-invokes asset-buyer (`sta_daily_digest`); price alerts wake it per ticker (`sta_flat_price_alert`). Asset-buyer has no trade cron. Overnight/BOATS closed. |
