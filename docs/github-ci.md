...
- **Fan-out shape:** full code-affecting runs split into independent jobs: `static checks` (lint/types/Knip/SQL/static migration grants + Lambda bundle), `unit tests` (Vitest sharded 6 ways), `e2e tests` (Playwright sharded 3 ways), `app build`, and the final required `ci` aggregator. Every test still runs; sharding only partitions the file list. Each shard has its own runner-local Supabase stack and keeps the suite serial inside the shard (`fileParallelism: false` / Playwright `workers: 1`), so DB-sharing assumptions are not weakened. This lowers wall-clock time at the cost of more total runner minutes and more parallel image pulls.
