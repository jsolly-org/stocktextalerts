...
**Shared automation secret:** `REPO_AUTOMATION_TOKEN` (non-admin PAT; Contents + Pull requests + Issues + Actions write — merge is Contents-gated per GitHub’s fine-grained PAT docs). Used by janitor (checkout/push/`gh`) and by CI’s `merge-pr` job (squash-merge after green `ci`). That merge is attributed to the PAT user, so `push` → main CI → Deploy runs normally.
