...
| **Post-merge bot dispatch** | [`.github/workflows/post-merge-bot.yml`](../.github/workflows/post-merge-bot.yml) | PR closed on `main` (bot merges only) | Fallback only if a merge is still attributed to `github-actions[bot]`. Primary path: CI `merge-pr` squash-merges with `JANITOR_GITHUB_TOKEN` after green `ci` so `push` fires normally. |
...
When `JANITOR_GITHUB_TOKEN` is set, [`.github/workflows/auto-merge.yml`](../.github/workflows/auto-merge.yml) skips GitHub `--auto` (fine-grained PATs cannot call `enablePullRequestAutoMerge` — observed on #648). Instead, CI's `merge-pr` job squash-merges with that PAT after the required `ci` check is green. The land is attributed to the PAT identity, so `push` → main CI → Deploy runs normally. Without the PAT, auto-merge falls back to `GITHUB_TOKEN` `--auto`; those merges are `github-actions[bot]` and GitHub suppresses workflow runs from that token's events (including `push` and, in practice, `pull_request` closed — observed on #647), so Deploy never follows. [`.github/workflows/post-merge-bot.yml`](../.github/workflows/post-merge-bot.yml) remains a best-effort fallback. Human merges still use the normal `push` → CI path.
