...
**Shared automation secret:** `REPO_AUTOMATION_TOKEN` — classic PAT with `repo` (non-admin). Used by janitor (checkout/push/`gh`; falls back to legacy `JANITOR_GITHUB_TOKEN` until you delete it) and by CI's `merge-pr` job (squash-merge after green `ci`; **merge path requires `REPO_AUTOMATION_TOKEN`**, not the legacy FG janitor token). That merge is attributed to the PAT user, so `push` → main CI → Deploy runs normally.
