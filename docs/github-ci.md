...
- **Fan-out shape:** full code-affecting runs split into independent jobs: `static checks` (lint/types/Knip/SQL/static migration grants + Lambda bundle), `unit tests` (Vitest, 1 shard), `e2e tests` (Playwright, 1 shard), `app build`, and the final required `ci` aggregator. Parallelism now lives *inside* each job rather than across shards: Vitest runs test files concurrently and Playwright runs 2 workers, so a second shard would only re-pay setup. Each job still has its own runner-local Supabase stack. Branch protection is unaffected: it requires the `ci` aggregator, not shard names.
- **Shard-count budget:** shard counts are capped by fixed per-shard setup, not by test volume, so every change that makes setup cheaper *or the suite faster* moves the right answer. Measured with `blacksmith jobs steps` (p50, 2026-08): setup was ~4s runner/checkout + ~9s setup-node + ~19s `npm ci` + ~122s Supabase bootstrap = ~154s in front of ~82s of tests. History: 6 shards (~75% setup) → 4 → 2 (node_modules sticky disk + seed-only bootstrap cut setup to ~85s) → **1**, once in-job parallelism cut the unit suite ~330s → ~70s and e2e ~111s → ~68s. Splitting a 70s suite across two runners spends 85s of setup to save ~35s, so it stops paying. **Before changing a shard count, re-measure** (`blacksmith jobs steps <job_id>`) and check the projection against the current critical path: the ceiling is `max(static, unit, e2e, app build)`. Adding shards below that ceiling buys nothing but setup; more shards also means more concurrent Supabase stacks pulling from `public.ecr.aws`, which is what [`ci-db-retry.sh`](../scripts/db/ci-db-retry.sh) exists to absorb.
