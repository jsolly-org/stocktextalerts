...
| **Post-merge bot dispatch** | [`.github/workflows/post-merge-bot.yml`](../.github/workflows/post-merge-bot.yml) | PR closed on `main` (bot merges only) | Fallback only: if a merge is still attributed to `github-actions[bot]`, try to `workflow_dispatch` **CI** on `main`. Prefer arming auto-merge with `JANITOR_GITHUB_TOKEN` so merges create a normal `push` (primary path). |
...
The auto-merge workflow calls `gh pr merge --auto --squash` for same-repo, non-draft, non-Dependabot PRs; GitHub merges when all required checks pass. It prefers `JANITOR_GITHUB_TOKEN` (same PAT janitor uses) so the squash merge is **not** attributed to `github-actions[bot]` and creates a normal `push` → CI → Deploy chain. A bare `GITHUB_TOKEN` arm still merges as the Actions bot; GitHub then suppresses workflow runs from that token's events (including `push` **and**, in practice, `pull_request` closed — observed on #647), so Deploy never follows. [`.github/workflows/post-merge-bot.yml`](../.github/workflows/post-merge-bot.yml) remains a best-effort fallback `workflow_dispatch` of CI when a bot merge *does* emit `closed`. Human merges still rely on the normal `push` → CI path. `/ship` may still call `--auto` immediately for a faster arm — redundant here, not required.
