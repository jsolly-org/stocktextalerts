...
- **Fan-out shape:** full code-affecting runs split into independent jobs: `static checks` (lint/types/Knip/SQL/static migration grants + Lambda bundle), `unit tests` (Vitest sharded 2 ways), `e2e tests` (Playwright sharded 2 ways), `app build`, and the final required `ci` aggregator. Every test still runs; sharding only partitions the file list. Each shard has its own runner-local Supabase stack and keeps the suite serial inside the shard (`fileParallelism: false` / Playwright `workers: 1`), so DB-sharing assumptions are not weakened. This lowers wall-clock time at the cost of more total runner minutes and more parallel image pulls. A 3-way E2E shard trial regressed wall-clock because every E2E runner still pays browser/dependency setup, so keep E2E at 2 shards unless new timing data says otherwise. A limited Vitest file-parallelism trial was faster but exposed DB-ordering assumptions/timeouts in integration tests, so unit shards remain file-serial until those tests are isolated more deliberately.
- **Shard-count budget:** shard counts are capped by fixed per-shard setup, not by test volume, so every change that makes setup cheaper moves the right answer. Measured with `blacksmith jobs steps` (p50, 2026-08): setup was ~4s runner/checkout + ~9s setup-node + ~19s `npm ci` + ~122s Supabase bootstrap = ~154s in front of ~82s of tests, against ~330s of total unit-test work and ~167s of total E2E work. History: 6 shards (~75% setup) → 4 shards → **2 shards**, the last step taken once the node_modules sticky disk and the seed-only bootstrap cut setup to ~85s, which puts a 2-way shard (~165s of tests, ~250s/job) at or under the e2e shards. **Before changing a shard count, re-measure** (`blacksmith jobs steps <job_id>`) and check the projection against the current critical path: the ceiling is `max(static, unit, e2e, app build)`, and today that is the e2e shards. Adding shards below that ceiling buys nothing but setup; more shards also means more concurrent Supabase stacks pulling from `public.ecr.aws`, which is what [`ci-db-retry.sh`](../scripts/db/ci-db-retry.sh) exists to absorb.
...
- **Caching:** `setup-node` npm cache stays on the upstream action, since Blacksmith transparently redirects it to its colocated cache (the `useblacksmith/cache` / `setup-node` forks are archived). Playwright browsers and `node_modules` use **sticky disks** instead of `actions/cache`: both are large binary trees where a tarball download + extract costs more than it saves, while a sticky disk hot-mounts in ~3s with no extraction. `node_modules` (1.1 GB unpacked) is keyed by OS+arch only, *not* by lockfile hash, because a hashed key hands every dependency bump an empty disk. [`scripts/ci-npm-install.sh`](../scripts/ci-npm-install.sh) instead stamps the lockfile hash inside the disk and reruns `npm ci` (~19s) whenever it or `node_modules/.bin/tsx` is missing, so a hit is exactly the tree `npm ci` would have produced. The npm cache stays on `setup-node` as the miss-path accelerator.
