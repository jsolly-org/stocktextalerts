...
| **Ensure main CI** | [`.github/workflows/ensure-main-ci.yml`](../.github/workflows/ensure-main-ci.yml) | Cron every 10 min + manual | If main tip has no green/in-flight CI, `workflow_dispatch` CI (allowed under the recursion guard). Unblocks Deploy after bot auto-merges. |
...
The auto-merge workflow calls `gh pr merge --auto --squash` with `GITHUB_TOKEN` for same-repo, non-draft, non-Dependabot PRs; GitHub merges when required checks pass. Those lands are attributed to `github-actions[bot]` and **do not** create `push` (or reliably `pull_request` closed) workflow runs — GitHub's recursion guard (observed on #647: post-merge-bot never started; Deploy stalled). Fine-grained PATs cannot `enablePullRequestAutoMerge` or `mergePullRequest` either (observed on #648), so the recovery path is [`.github/workflows/ensure-main-ci.yml`](../.github/workflows/ensure-main-ci.yml): every ~10 minutes it `workflow_dispatch`es CI on `main` when the tip has no green/in-flight CI run (`workflow_dispatch` is an allowed exception). Deploy then follows via `workflow_run`. Human merges still use the normal `push` → CI path. `/ship` may still call `--auto` immediately for a faster arm — redundant here, not required.
