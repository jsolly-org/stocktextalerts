...
- **Fan-out shape:** full code-affecting runs split into independent jobs: `static checks` (lint/types/Knip/SQL/static migration grants + Lambda bundle), `unit tests` (Vitest sharded 4 ways, 2 workers per shard), `e2e tests` (Playwright sharded 2 ways), `app build`, and the final required `ci` aggregator. Every test still runs; sharding only partitions the file list. Each shard has its own runner-local Supabase stack; unit shards opt into limited Vitest file parallelism after their fresh `db:reset`, while E2E shards keep Playwright `workers: 1`. The unit setup skips the per-worker global user purge only in these fresh CI shards to avoid deleting another worker's active test users; local/repeated-run cleanup remains the default. This lowers wall-clock time at the cost of more total runner minutes and more parallel image pulls. A 3-way E2E shard trial regressed wall-clock because every E2E runner still pays browser/dependency setup, so keep E2E at 2 shards unless new timing data says otherwise.
