...
## Shared local Supabase stack — follow-ups (surfaced 2026-06-21)

Context: collapsed per-worktree stacks onto one shared stack serialized by `<git-common-dir>/test.lock` (see `docs/plans/2026-06-21-shared-local-supabase-stack.md`). The races below were flagged by the impl review; the last two items came out of a deep-research pass and are deferred because they're not clean low-risk wins for this stack.

- [ ] **Permanent fix for the Postgres collation-version wedge.** `supabase start` once failed with `template1 collation version mismatch` (153.120 vs OS 153.121), recovered by deleting `supabase_db`/`supabase_storage` volumes. The in-place fix is `REINDEX DATABASE` then `ALTER DATABASE … REFRESH COLLATION VERSION` on the app DB *and* `template1` (start clones from it). BUT our collation is ICU-style (`153.x`) while documented fixes are glibc, and image-pin efficacy (`[db] major_version` vs digest) is unestablished — verify on the ICU collation before scripting an auto-refresh into `db:start`.
- [ ] **Trim Storage/imgproxy from the shared stack.** Backups use S3, not Supabase Storage (zero `.storage.` call sites), so `storage` + `imgproxy` are candidates for `enabled = false` — another container or two off the stack. Left enabled this round (conservative); validate the full suite + `astro dev` after disabling.
- [ ] **Faster `db:reset`** is mostly N/A: transactional `BEGIN/ROLLBACK` (~98% faster) needs a direct `pg` client, but this suite uses supabase-js over HTTP; template cloning skips GRANTs the privilege contract needs. Revisit only if reset time becomes a real bottleneck.

- [ ] **`db:start` stale-recovery can yank the shared containers under a running suite.** `scripts/db/start.ts` is intentionally not lock-held (it's on `predev`), but its stale-state branch runs `supabase stop` + `start`. If that fires while another worktree holds the test lock mid-query, that suite sees a transient blip / confusing failure. Window is narrow (only on the stale-state recovery path). Consider a non-blocking `test.lock` check in the recovery branch that prints a loud banner and exits rather than stopping containers in use.
- [ ] **`test.lock` has a TOCTOU window in stale-takeover.** `tests/lock.ts` checks `isProcessAlive()` then writes the lock; a holder dying in that gap (e.g. Ctrl+C on a slow `db:reset` before its SIGINT handler runs) lets a second worktree reclaim and start a concurrent reset. Low probability + reset is idempotent, so blast radius is limited. The real fix is `flock(2)`/`fcntl` advisory locking (auto-released on process exit) instead of the JSON sentinel.
