...
Every log line includes a `release` field stamped with the first 12 chars of the deployed git SHA (e.g. `"b6c74e56a231"`), or `"<sha>-dirty"` when the working tree was modified at build time. On Vercel CLI deploys without `VERCEL_GIT_COMMIT_SHA`, the value falls back to `"dev"`. This field is generated by `scripts/gen-release-id.ts` at build time and is ignored by the shared-infra enricher (additive only).
