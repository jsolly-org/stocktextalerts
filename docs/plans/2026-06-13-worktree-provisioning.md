...
**If `npm ci` fails with EACCES on `~/.npm`:** the npm cache is root-owned (a past `sudo npm`). Fix once: `sudo chown -R $(id -u):$(id -g) ~/.npm`. (`dotagents`' `doctor-agents.sh` warns about this.) Under the Claude Code command sandbox `~/.npm` is not writable, so `worktree:init` redirects the npm cache to `$TMPDIR` (`npm_config_cache`) — `npm ci` runs in-sandbox with no `~/.npm` sandbox grant (that grant was considered and rejected). Run it online (never `--offline`): a cold `$TMPDIR` cache plus offline mode hard-fails. A bare manual `npm ci` should pass `--cache="$TMPDIR/.npm-cache"` for the same redirect.
