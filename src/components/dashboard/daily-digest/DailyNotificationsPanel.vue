				...
				<div class="flex flex-col gap-2 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
					...
					<div class="min-w-0">
						...
						<p
							...
							class="text-sm text-body-secondary mt-0.5"
						>
							Tomorrow's up/down per tracked asset, plus curated
...
/** Telegram is selectable when linked and not globally muted (`telegram_opted_out`). */
...
const hasAnyAssetEventsOptionEnabled = computed(
	() =>
		...
		user.value.asset_events_include_short_interest_email ||
		props.telegramPrefs.short_interest === true,
);
...
watch(savedData, (newData) => {
	...
	user.value = {
		...
		...(newData.asset_events_include_short_interest_email !== undefined && {
			asset_events_include_short_interest_email:
				newData.asset_events_include_short_interest_email,
		}),
