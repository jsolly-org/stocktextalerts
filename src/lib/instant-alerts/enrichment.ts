import { type CompanyNewsItem, fetchCompanyNews } from "../finnhub-extras";
import type { ExtendedAssetQuote } from "../price-fetcher";
import type { AnomalyResult } from "./anomaly-detection";
import { generateInstantAlertSummary } from "./grok-summary";
...
export async function enrichAlert(options: {
	...
	if (news.length > 0) {
		aiSummary = await generateInstantAlertSummary({
			symbol,
			priceContext,
			signalContext,
