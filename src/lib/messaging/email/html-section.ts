...
/**
 * Sanitize, deduplicate, and cap citation URLs.
 *
 * Only http(s) URLs are kept; duplicates are removed while preserving order.
 */
...
/**
 * Render a "Sources" section listing citation URLs as clickable links.
 *
 * Returns an empty string when there are no valid citations.
 */
export function renderCitationsSection(citations: string[]): string {
	const clean = sanitizeCitations(citations);
	if (clean.length === 0) return "";

	const linkStyle = `${LINK_STYLE} word-break: break-all;`;
	const items = clean
		.map((url) => {
			...
			return `<li style="margin-bottom: 4px;"><a href="${escaped}" style="${linkStyle}" target="_blank" rel="noopener noreferrer">${display}</a></li>`;
		})
		.join("");

	return `<h3 style="margin: 16px 0 6px; font-size: 14px;">🔗 Sources</h3><ul style="margin: 0; padding: 0 0 0 20px; font-size: 13px; line-height: 1.6;">${items}</ul>`;
}

/**
 * Format citations as plain text for the text version of the email.
 *
 * Returns an empty string when there are no valid citations.
 */
export function formatCitationsText(citations: string[]): string {
	const clean = sanitizeCitations(citations);
	if (clean.length === 0) return "";
	return `\n🔗 Sources\n${clean.map((url) => `- ${url}`).join("\n")}`;
}
