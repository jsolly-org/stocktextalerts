...
/**
 * Fetch users eligible for daily add-ons processing.
 * In force mode, users without `add_ons_next_send_at` may be returned.
 */
export async function fetchDailyAddOnsUsers(options: {
	...
	for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
		let query = options.supabase
			...
			.eq("add_ons_notifications_enabled", true)
