...
while IFS='=' read -r _key _value; do
  ...
  # Skip AWS_* credential/region selectors. .env.local may set AWS_PROFILE for local
  # tooling, but this full infra deploy must run under the operator's admin
  # session — importing a non-admin profile here silently forces sam onto the
  # wrong role and it fails closed on cloudformation:CreateChangeSet. SAM still
  # reads creds from the operator's environment/SSO.
  [[ "$_key" == AWS_* ]] && continue
  export "$_key=$_value"
done < "$_ENV_FILE"
