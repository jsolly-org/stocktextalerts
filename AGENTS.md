...
- **Local-only files (gitignored, created during setup):** `.env.local` holds the local Supabase keys (pulled from `supabase status`), `EMAIL_SMTP_HOST=localhost` (→ Mailpit), and a generated `DEFAULT_PASSWORD`. `scripts/data/users.json` (copied from `scripts/data/sample-users.json`) seeds dev logins; `supabase/seed.sql` is generated by `npm run db:generate-seed`.
...
- **Quick logged-in session (hello-world):** sign in at `/auth/signin` as `dev@example.com` with the `DEFAULT_PASSWORD` value from `.env.local` — this seed user is pre-confirmed and pre-approved, so it lands straight on `/dashboard` (no Mailpit/admin-approval step, which new registrations require). Track a stock via the "My Watchlist" search to exercise `POST /api/assets/update`.
